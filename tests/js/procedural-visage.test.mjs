...
test("visage exposes the full controls contract plus butler extras", () => {
  ...
  assert.equal(
    rig.controls.hairRoot.children.filter((child) => child.name === "hairRidge").length,
    7,
    "high quality keeps the swept hairstyle legible",
  );
  assert.ok(rig.controls.earLeft.children.length >= 4, "left ear includes anatomical contours");
  assert.ok(rig.controls.earRight.children.length >= 4, "right ear includes anatomical contours");
  rig.dispose();
});
