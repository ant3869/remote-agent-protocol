...
processor_path = Path("remote_agent_protocol/session_processors.py")
test_path = Path("tests/test_session_processors.py")
...
subprocess.run(["python", "-m", "ruff", "format", str(processor_path), str(test_path)], check=True)
subprocess.run(
    [
        "python", "-m", "pytest",
        "tests/test_avatar_audio.py", "tests/test_session_processors.py",
...
subprocess.run(
    ["git", "add", str(processor_path), str(test_path), ".github/avatar_tasks/task3.py"],
    check=True,
)
