...
(avatar / "procedural-butler.js").write_text(
...
subprocess.run(["git", "add", str(avatar), str(test_path), ".github/avatar_tasks/task8.py"], check=True)
subprocess.run(["git", "config", "user.name", "github-actions[bot]"], check=True)
subprocess.run(["git", "config", "user.email", "41898282+github-actions[bot]@users.noreply.github.com"], check=True)
subprocess.run(["git", "commit", "-m", "feat(avatar): render procedural butler companion"], check=True)
subprocess.run(["git", "push", "origin", "HEAD:feature/animated-butler-avatar"], check=True)
print("TASK 8 DONE: procedural butler and Three.js scene committed")
