...
export function createButlerMonocle(THREE, { kit, trackGeometry, quality = "high", anchor }) {
  ...
  // Monocle materials skip most of the scanline dimming so the signature
  // feature stays crisp against the face.
  const ringMaterial = kit.line(1, { scanlineStrength: 0.15 });
  const secondaryMaterial = kit.line(0.3, { accent: true, scanlineStrength: 0.2 });
  const tickMaterial = kit.line(0.28, { scanlineStrength: 0.2 });
  const reticleMaterial = kit.line(0.75, { accent: true, scanlineStrength: 0.2 });
  const orbiterMaterial = kit.glow(0.85, 0.6, { accent: true });
  const glowMaterial = kit.glow(0.08, 0.12);
  const connectorMaterial = kit.line(0.34);
  ...
  const glow = new THREE.Mesh(trackGeometry(new THREE.PlaneGeometry(0.42, 0.42)), glowMaterial);
  ...
  if (!simplified) {
    ringSecondary.add(line(arcPoints(THREE, R + 0.028, 0.35, 0.35 + TAU * 0.62, 40), secondaryMaterial));
    ringSecondary.add(line(arcPoints(THREE, R + 0.028, 0.35 + TAU * 0.72, 0.35 + TAU * 0.82, 10), secondaryMaterial));
  }
  ...
  const tickPoints = [];
  const tickCount = simplified ? 6 : 8;
  for (let i = 0; i < tickCount; i += 1) {
    const angle = (i / tickCount) * TAU;
    const isMajor = i % 4 === 0;
    const r0 = R + 0.04;
    const r1 = r0 + (isMajor ? 0.024 : 0.012);
    tickPoints.push(
