...
function sculptDirection(direction, target) {
  ...
  target.set(direction.x * radius * 0.9, direction.y * radius * 1.06, direction.z * radius * 0.82);
  const jawTaper = 1 - 0.23 * smoothstep(-0.12, -0.85, direction.y) * (0.35 + 0.65 * front);
  const boxiness = 0.96 + 0.1 * smoothstep(0.25, 0.82, Math.abs(direction.y));
  target.x *= jawTaper * boxiness;
  return target;
}
...
export function createButlerHead(THREE, { kit, trackGeometry, quality = "high" }) {
  ...
  const wireMaterial = kit.line(0.018);
  const vertexMaterial = kit.points(0.07, 2.2);
  ...
  head.add(headPoints);
  head.add(new THREE.Mesh(headShellGeometry, kit.shell(0.72, 0.085)));
  ...
  jaw.add(new THREE.Mesh(jawShellGeometry, kit.shell(0.66, 0.07)));
  ...
  const mustacheFillMaterial = kit.shell(1, 0.14);
  const mustacheEdgeMaterial = kit.line(1);
  const tipMaterial = kit.glow(0.55, 0.6);
  const buildMustache = (side, name) => {
    const group = new THREE.Group();
    group.name = name;
    group.position.set(0.012 * side, -0.125, 0.443);
    ...
    const thick = new THREE.Mesh(trackGeometry(new THREE.TubeGeometry(curve, 12, 0.015, 6, false)), mustacheFillMaterial);
  ...
  // --- Ears: elongated outer helix plus inner antihelix/concha contours.
  const earMaterial = kit.glow(0.11, 0.24);
  const earRingMaterial = kit.line(0.46);
  const earInnerMaterial = kit.line(0.3);
  const ears = [];
  for (const side of [-1, 1]) {
    const node = new THREE.Group();
    node.name = side < 0 ? "earLeft" : "earRight";
    node.position.set(0.475 * side, -0.045, 0.075);
    node.rotation.z = -0.04 * side;
    node.scale.y = 1.35;
    node.add(glowPlane(0.12, 0.22, earMaterial));
    const helix = [];
    for (let i = 0; i <= 28; i += 1) {
      const angle = (i / 28) * TAU;
      helix.push(new THREE.Vector3(Math.cos(angle) * 0.064, Math.sin(angle) * 0.12, 0));
    }
    node.add(lineFromPoints(helix, earRingMaterial, true));
    node.add(curveThroughEar(THREE, trackGeometry, [
      [-0.016 * side, 0.068], [0.018 * side, 0.035], [-0.024 * side, -0.005],
      [0.012 * side, -0.048], [-0.018 * side, -0.072],
    ], earInnerMaterial));
    node.add(curveThroughEar(THREE, trackGeometry, [
      [0.018 * side, 0.012], [-0.004 * side, -0.004], [0.02 * side, -0.028],
    ], earInnerMaterial));
    head.add(node);
    ears.push(node);
  }
  ...
  return {
    ...
    controls: {
      ...
      moustacheLeft, moustacheRight, earLeft: ears[0], earRight: ears[1],
    },
...
function curveThroughEar(THREE, trackGeometry, anchors, material) {
  const points = anchors.map(([x, y]) => new THREE.Vector3(x, y, 0.004));
  const curve = new THREE.CatmullRomCurve3(points);
  const geometry = trackGeometry(new THREE.BufferGeometry().setFromPoints(curve.getPoints(18)));
