...
import { createSpriteButler } from "./sprite-butler.js";
...
export async function createAvatarScene(host, settings) {
  ...
  let metadata = { model: null, fallback: "sprite-butler", scale: 1 };
  ...
  let rig;
  if (loaded.kind === "gltf") rig = adaptLoadedRig(loaded, metadata, THREE);
  else if (metadata.fallback === "procedural-butler") rig = createProceduralButler(THREE);
  else if (metadata.fallback === "sprite-butler") {
    try {
      rig = await createSpriteButler(THREE, {
        sprites: metadata.sprites,
        quality: settings.quality,
        reducedMotion: settings.effectiveReducedMotion,
      });
    } catch (error) {
      console.warn("Avatar sprites unavailable; using procedural fallback", error);
    }
  }
  rig ||= createProceduralVisage(THREE, {
    quality: settings.quality,
    reducedMotion: settings.effectiveReducedMotion,
  });
  ...
  const resize = () => {
    ...
    camera.position.set(0, target.y + 0.2 + tightness * 0.12, 3.2 - tightness * 0.25);
