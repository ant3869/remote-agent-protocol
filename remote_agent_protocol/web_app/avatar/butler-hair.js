...
function edgeTheta(azimuth) {
  ...
  let theta = Math.PI * (0.27 + 0.27 * (1 - front) / 2);
...
export function createButlerHair(THREE, { kit, trackGeometry, surfacePoint, quality = "high" }) {
  ...
  const liftAt = (azimuth, v) => {
    ...
    let lift = 0.022 + 0.018 * (1 - v);
    lift += 0.075 * gauss((v - 0.78) / 0.27) * frontness;
  ...
  const patchMaterial = kit.shell(0.74, 0.09);
  ...
  const rimMaterial = kit.line(0.62);
  ...
  const waveMaterial = kit.shell(0.9, 0.1);
  const waveEdgeMaterial = kit.line(0.8);
  const wavePath = [];
  for (let i = 0; i <= 10; i += 1) {
    ...
    wavePath.push(surfaceAt(azimuth, theta, 0.05 + 0.062 * Math.sin(Math.PI * Math.min(1, t * 1.25))));
  }
  const waveCurve = new THREE.CatmullRomCurve3(wavePath);
  group.add(new THREE.Mesh(trackGeometry(new THREE.TubeGeometry(waveCurve, 20, 0.062, 6, false)), waveMaterial));
  ...
  // Layered comb ridges give the front silhouette the concept's tidy,
  // sculpted locks instead of relying only on longitudinal flow lines.
  const ridgeMaterial = kit.line(1);
  const ridgeCount = quality === "low" ? 3 : quality === "medium" ? 5 : 7;
  for (let row = 0; row < ridgeCount; row += 1) {
    const path = [];
    for (let i = 0; i <= 12; i += 1) {
      const t = i / 12;
      const azimuth = -1.12 + t * 2.24 + 0.12 * Math.sin(Math.PI * t);
      const theta = Math.PI * (0.18 + row * 0.027) - 0.035 * Math.sin(Math.PI * t);
      path.push(surfaceAt(azimuth, theta, 0.05 + (ridgeCount - row) * 0.004));
    }
    const ridge = new THREE.Line(
      trackGeometry(new THREE.BufferGeometry().setFromPoints(new THREE.CatmullRomCurve3(path).getPoints(28))),
      ridgeMaterial,
    );
    ridge.name = "hairRidge";
    group.add(ridge);
  }

  // --- Flow strands: bright combed-back lines from the hairline over the
  // crown to the nape, drifting toward the sweep side.
  const strandMaterial = kit.line(0.2);
  ...
  for (const start of strandStarts) {
    ...
    for (let i = 0; i <= 8; i += 1) {
      ...
      const theta = edgeTheta(start) * (1 - t) * (1 - t)
        + Math.PI * 0.1 * 2 * t * (1 - t)
        + Math.PI * 0.58 * t * t;
      path.push(surfaceAt(azimuth, Math.max(Math.PI * 0.07, theta), 0.045 + 0.02 * Math.sin(Math.PI * t)));
    }
    ...
    strand.name = "hairFlow";
    group.add(strand);
  }

  // --- Sparse point accents on the patch (skipped on low quality).
  if (quality !== "low") {
    const accentCount = quality === "high" ? 70 : 36;
  ...
  const update = (seconds) => {
    ...
    strandMaterial.uniforms.uOpacity.value = 0.16 + 0.06 * Math.sin(seconds * 0.9);
    waveEdgeMaterial.uniforms.uOpacity.value = 0.74 + 0.12 * Math.sin(seconds * 0.7 + 1.4);
  };
