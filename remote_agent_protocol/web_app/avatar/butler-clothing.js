...
export function createButlerClothing(THREE, { kit, trackGeometry, quality = "high" }) {
  ...
  bowTieRoot.scale.setScalar(1.42);
  group.add(bowTieRoot);
  const bowFillMaterial = kit.shell(0.9, 0.1);
  const bowEdgeMaterial = kit.line(1);
  const buildWing = (side) => {
    ...
    const outerTop = new THREE.Vector3(0.12 * side, 0.052, -0.012);
    const outerBottom = new THREE.Vector3(0.12 * side, -0.052, -0.012);
