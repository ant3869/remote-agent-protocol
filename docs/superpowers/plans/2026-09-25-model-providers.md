...
- **Task 5 wire-format casing.** `ProviderConfig.to_dict()`/`TestResult.to_dict()`/`RoleChainEntry.to_dict()` use snake_case field names (matching the on-disk JSON schema), and `_providers_payload()` spreads them as-is rather than remapping to camelCase like the rest of `/api/status`. This is a real inconsistency with the app's usual wire convention; left as-is rather than adding a remapping layer, since `app.js` reads the fields under their actual names (`provider.base_url`, `entry.provider_id`, `result.latency_ms`) and there was no test coverage riding on camelCase for these fields. Worth a follow-up cleanup pass, not blocking.
- **Task 6 role-chain editor.** The spec describes an "ordered chain editor (add from the browser, reorder, remove)". Implemented add/remove; reordering is not implemented (a chain is appended-to and removed-from only, not drag-or-arrow-reordered). Multi-hop fallback chains are rare in practice (most operators assign one provider+model per role), and building a reorder control was judged lower value than the rest of the UI given the remaining scope. Tracked here rather than silently dropped.
- **Task 6 model browser scope.** The model browser lists cached catalog models with search-filter and a manual-ID fallback, and a Test button per row (badges render from `modelTests` once tested). It does not implement virtualization/pagination beyond a 200-row cap -- acceptable for now since OpenRouter-scale catalogs (hundreds of models) still render fine unpaginated in a scrollable list, per the CSS `max-height` + `overflow-y: auto` on `.model-browser-list`.
- **Task 6 status view.** The spec's "Status view" describes extending the existing "Persona orchestration" panel to show which provider/model answered each role's last turn. Not implemented -- that panel belongs to a different, pre-existing feature (`_orchestration_payload`/persona page) and wiring role-assignment awareness into it was judged out of scope for this pass; the Models & providers panel's own provider/model test badges are the substitute for now.
